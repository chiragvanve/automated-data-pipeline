...
# --- NEW: Pull AI Prediction & Accuracy ---
prediction_val = df['Prediction'].iloc[-1]
accuracy_val = df['Accuracy'].iloc[-1]
# -------------------------------------------

# 4. Sidebar - Model Performance
st.sidebar.header("🤖 AI Model Health")
st.sidebar.metric("Confidence Score", f"{accuracy_val:.1f}%")
st.sidebar.progress(accuracy_val / 100)

# Add a status message based on accuracy
if accuracy_val > 80:
    st.sidebar.success("Strong Trend: High Reliability")
elif accuracy_val > 50:
    st.sidebar.info("Moderate Trend: Use Caution")
else:
    st.sidebar.warning("Choppy Market: Low Reliability")

# 5. Draw the KPI boxes at the top
...
# 7. Show the underlying data table
